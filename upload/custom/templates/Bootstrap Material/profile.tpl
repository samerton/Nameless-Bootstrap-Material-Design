		  ...
		  {nocache}
			...
			{if $MOD_OR_ADMIN ne true}
                <a href="#" data-toggle="modal" data-target="#blockModal"
                   class="btn btn-outline-danger btn-lg active"><i class="fa fa-ban fa-fw"></i></a>
            {/if}
			<a href="{$MESSAGE_LINK}" class="btn btn-outline-secondary btn-lg active"><i
                        class="fa fa-envelope fa-fw"></i></a>
			{if isset($RESET_PROFILE_BANNER)}
                <a href="{$RESET_PROFILE_BANNER_LINK}" class="btn btn-red btn-lg" rel="tooltip"
                   data-title="{$RESET_PROFILE_BANNER}">
...
{else}
		    ...
		    <button type="button" class="btn btn-outline-info btn-lg active" data-toggle="modal"
                    data-target="#imageModal"><i class="fas fa-image fa-fw" aria-hidden="true"></i></button>
		  </div>
                  {/if}
              {/if}
          {/nocache}
            ...
            <div class="card-body">
                ...
                <div class="tab-content">
                            ...
                            {foreach from=$WALL_POSTS item=post}
                                <div class="card card-default">
                                    <div class="card-header">
                                        <img class="rounded-circle" style="max-height:25px; max-width:25px;"
                                             src="{$post.avatar}"/> <a data-poload="{$USER_INFO_URL}{$post.user_id}"
                                                                       data-html="true" data-placement="top"
                                                                       href="{$post.profile}"
                                                                       style="{$post.user_style}">{$post.nickname}:</a>
                                        <span class="float-md-right"><span rel="tooltip"
                                                                       data-original-title="{$post.date}">{$post.date_rough}</span></span>
                                    </div>
                                    ...
                                    <div class="card-footer">
                                        <a href="{if $post.reactions_link ne "#"}{$post.reactions_link}{else}#{/if}"
                                           class="pop"
                                           data-content='{if isset($post.reactions.reactions)} {foreach from=$post.reactions.reactions item=reaction name=reactions}<a href="{$reaction.profile}" style="{$reaction.style}"><img class="rounded" src="{$reaction.avatar}" alt="{$reaction.username}" style="max-height:30px; max-width:30px;" /> {$reaction.nickname}</a>{if !$smarty.foreach.reactions.last}<br />{/if}{/foreach} {else}{$post.reactions.count}{/if}'><i
                                                    class="fa fa-thumbs-up"></i> {$post.reactions.count} </a> | <a
                                                href="#" data-toggle="modal" data-target="#replyModal{$post.id}"><i
                                                    class="fa fa-comments"></i> {$post.replies.count}</a>
                                        <span class="float-md-right">
				  {if (isset($CAN_MODERATE) && $CAN_MODERATE eq 1) || $post.self eq 1}
                      ...
                      <a href="#" data-toggle="modal" data-target="#editModal{$post.id}">{$EDIT}</a>
                       |
                  ...
                  {/if}
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="reactModal{$post.id}Label">{$REACTIONS_TITLE}</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                            aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                ...
                                <!-- Replies modal -->
                                <div class="modal fade" id="replyModal{$post.id}" tabindex="-1" role="dialog"
                                     aria-labelledby="replyModal{$post.id}Label" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"
                                                    id="replyModal{$post.id}Label">{$REPLIES_TITLE}</h5>
                                                <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                                ...
                                                {if isset($post.replies.replies)}
                                                    {foreach from=$post.replies.replies name=replies item=reply}
                                                        <img src="{$reply.avatar}" alt="{$reply.username}"
                                                             style="max-height:20px; max-width:20px;" class="rounded"/>
                                                        <a href="{$reply.profile}"
                                                           style="{$reply.style}">{$reply.nickname}</a>
                                                        &raquo;
                                                        <span class="float-md-right">
					    <span rel="tooltip" title="{$reply.time_full}">{$reply.time_friendly}</span>
                                                ...
                                                <form action="" method="post">
                                                    <textarea class="form-control" name="reply"
                                                              placeholder="{$NEW_REPLY}"></textarea>
                                                    <input type="hidden" name="token" value="{$TOKEN}">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="card">
                                    <div class="card-body">
                                        <center>
                                            <img {if isset($ABOUT_FIELDS.minecraft)}src="{$ABOUT_FIELDS.minecraft.image}{else}class="
                                                 rounded" style="max-height:75px;max-width:75px;" src="{$AVATAR}{/if}"
                                            alt="{$USERNAME}" onerror="this.style.display='none'" />
                                            <h2{if $USERNAME_COLOUR != false} style="{$USERNAME_COLOUR}"{/if}>{$NICKNAME}</h2>
                                            {$USER_TITLE}
                                        </center>
                                        <hr/>
                                        <ul>
                                            <li><strong>{$ABOUT_FIELDS.registered.title}</strong> <span rel="tooltip"
                                                                                                        title="{$ABOUT_FIELDS.registered.tooltip}">{$ABOUT_FIELDS.registered.value}
                                            </li>
                                            <li><strong>{$ABOUT_FIELDS.last_seen.title}</strong> <span rel="tooltip"
                                                                                                       title="{$ABOUT_FIELDS.last_seen.tooltip}">{$ABOUT_FIELDS.last_seen.value}
                                            </li>
                                            <li>
                                                <strong>{$ABOUT_FIELDS.profile_views.title}</strong> {$ABOUT_FIELDS.profile_views.value}
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                                        ...
                                        {if !isset($NO_ABOUT_FIELDS)}
                                            {foreach from=$ABOUT_FIELDS key=key item=field name=about_foreach}
                                                {if is_numeric($key)}
                                                    <h3>{$field.title}</h3>
                                                    <p>{$field.value}</p>
                                                    {if !$smarty.foreach.about_foreach.last}
                                                        <hr/>
                                                    {/if}
                                                {/if}
                                            {/foreach}
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" onclick="document.updateBanner.submit()"
            ...
            <div class="modal fade" id="blockModal" tabindex="-1" role="dialog" aria-labelledby="blockModalLabel"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"
                                id="blockModalLabel">{if isset($BLOCK_USER)}{$BLOCK_USER}{else}{$UNBLOCK_USER}{/if}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
